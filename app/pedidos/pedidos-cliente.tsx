    ...
    <div>
      <div className="mb-4 flex flex-wrap gap-3">
        {Object.entries(COLORES).map(([k, c]) => {
          const I = c.Icono
          return <span key={k} className="flex items-center gap-2 text-sm text-neutral-700"><span className={'flex h-5 w-5 items-center justify-center rounded ' + c.bg}><I className="h-3 w-3 text-white" /></span>{c.etiqueta}</span>
        })}
      </div>
      <div className="mb-5 flex flex-wrap gap-2">
        {zonas.map((z) => {
          const I = iconoZona(z.nombre); const activa = zonaSel === z.id
          return <button key={z.id} onClick={() => setZonaSel(z.id)} className={'flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold ' + (activa ? 'bg-neutral-900 text-white' : 'border border-neutral-300 bg-white text-neutral-700 hover:bg-neutral-100')}><I className="h-4 w-4" /> {z.nombre}</button>
        })}
      </div>
      {delArea.length === 0 ? <p className="text-neutral-500">No hay mesas en esta área.</p> : (
          ...
          {delArea.map((m) => {
            ...
            return (
              <button key={m.id} onClick={() => router.push(`/pedidos/${m.id}`)}
                ...
                {t && <span className="mt-1 text-xs opacity-90">⏱ {t}</span>}
              </button>
            )
          })}
        </div>
